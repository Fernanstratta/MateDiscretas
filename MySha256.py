...
def parse_blocks(message):
    """Divide the message into 512-bit (64-byte) blocks."""
    return [message[i:i+64] for i in range(0, len(message), 64)]

def create_message_schedule(block):
    """Create a 64-entry message schedule array from a 512-bit block."""
    W = [0] * 64
    for t in range(16):
...
def compress_block(W, H):
    """Perform the main hash computation on a single block."""
    a, b, c, d, e, f, g, h = H
    for t in range(64):
...
print(f"SHA-256 hash of '{input_string}': {sha256(input_string)}")
